...
The prospect was found through a currently active Meta ad. Never claim exact spend, targeting, performance, leads, or results. Treat ad longevity and creative count only as evidence that the firm is actively buying attention. Use only supplied firm, market, and ad evidence. Never invent a practice area, result, statistic, or personal fact.
...
export async function POST(request: Request) {
  try {
    const { lead, firstName } = await request.json() as { lead: Lead; firstName: string }; if (!lead || !firstName?.trim()) return NextResponse.json({ error: "Choose a firm and enter a verified contact first name." }, { status: 400 });
